...
export function positionUnknownGeneration(otherGenerationNodes: FamilyMember[] | undefined, minGeneration: number) {
    if (!otherGenerationNodes || otherGenerationNodes.length === 0) {
        return [];
    }

    const baseY = (minGeneration - 1) * (GENERATION_HEIGHT + NODE_HEIGHT);
    const unknownGenerationNodes = otherGenerationNodes.map((node, i) => {
        const x = calcX(i % MAX_IN_ROW);
        const othersY = baseY - Math.floor(i / MAX_IN_ROW) * (NODE_HEIGHT + GENERATION_HEIGHT) * 0.7;
...
export function positionAndBuildFamilyTree(
    ...
    const sortedGenerations = Object.keys(innerFamiliesPerGeneration)
        .map(Number)
        .filter(g => g !== OTHERS_GENERATION)
        .sort((a, b) => a - b);

    for (const currentGeneration of sortedGenerations) {
